...
Here we're just checking if a player has hit Escape button and exit game if so. Final match arm is for every other 
events, nothing fancy here - just asking engine to continue listening for new events.
...
field, apply `Cylinder` collider and adjust its height and radius.
...
Also change `let mut game = Game::new();` to this:
...
in the `create_skybox` function. Also modify player creation in `Game::new` to this
...
[GitHub](https://github.com/mrDIMAS/rg3d-tutorials). In the next tutorial we'll start adding weapons and bots.

Discussion: Reddit, [Discord](https://discord.gg/xENF5Uh).
